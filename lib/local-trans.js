...
ra2webInjection.translationMap = {
  当前游戏模式不可用:
    '当前游戏模式不可用，请微信关注公众号 项码士 阅读里面文章获取解决方案！',
  'Americas & Europe': '北美-欧洲大区',
  'South-East Asia': '大中华区',
  'https://xwis.net/dl/Red-Alert-2-Multiplayer.exe': 'dom-node:discordlink',
  'Running version PvPGN 1.99.7.1.1-PRO.':
    '严禁辱骂、发表种族歧视言论、恶意骚扰行为，轻则禁言，重则永久封号',
  'Ladder Season 4 is live. Play a "Quick Match" and get your rank badge now!':
    '第四赛季已经开赛！！主菜单选择 排位赛 尽情取得自己的徽章吧！！',
  ...
  'Join us on Discord: https://discord.gg/uavJ34JTWY':
    '网页红色警戒问题反馈，请微信关注公众号 项码士 获取',
  您加入了美國指揮中心頻道:
    '您已返回房间大厅。问题反馈、游戏交流，欢迎微信关注公众号 项码士',
  您已經與伺服器斷線了:
    '您已经与服务器断开连接，微信关注公众号 项码士 阅读里面文章获取解决方案',
  自訂戰役: '定制对局',
  基地重新部署: '基地可重新部署',
  升級工具箱: '随机宝箱',
  部隊數: '初始作战单位',
...
function handleButtonClick() {
  ...
  switch ((console.log('点击了按钮:', e), e)) {
    case '家':
      ...
      break
    case '事':
      window.ra2webInjection.CdApi.battleControl.executeKeyCommand(
        'CenterOnRadarEvent'
      )
      break
    case '斗':
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  ;(toolBoxInTouch = !1),
    ...
    (ctrlShiftAltContainer = document.getElementById(
      'ctrl-shift-alt-container'
    )),
  ...
  tipsSwitchButton.addEventListener('click', ra2webGlobalToolFunc.switchTips)
  ...
  const jihuAdLoad = function () {
    ...
    link.target = '_blank'
    link.innerHTML =
      '感谢【极狐】赞助<br />网页红色警戒CDN资源<br />使加载速度极大提升'
      ...
      ra2webInjection.joy.on('start move end', function (t, i) {
        ...
        switch (
          ...
          case 'start':
            ...
            window.ra2webInjection.CdApi.battleControl.requestPan(
              1.3 * l,
              1.3 * c
            )
            break
...
function sendPostRequest(url) {
  return fetch(url).catch((error) =>
    console.error(
      'Error in sending monitor data, please 微信关注公众号 项码士 反馈问题！:',
      error
    )
  )
}
...
fetch(`//${monitorTargetSite}/manifest.json`)
  .then((response) => {
    return sendPostRequest(
      `//monitor-agent.ra2web.cn/api/can-access-site/${monitorTargetSite}/monitor-log-metrics?a=114514`
    )
  })
  .catch((error) => {
    return sendPostRequest(
      `//monitor-agent.ra2web.cn/api/can-access-site/${monitorTargetSite}/monitor-log-metrics?a=1919810`
    )
  })
